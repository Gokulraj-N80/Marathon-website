    ...
    <section className="py-8 md:py-28 bg-background dots-pattern">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-2xl mx-auto mb-6 md:mb-16">
          <p className="text-navy dark:text-primary-light font-semibold text-xs tracking-widest uppercase">Every Participant Gets</p>
          <h2 className="mt-2 text-xl md:text-5xl font-extrabold text-charcoal dark:text-white tracking-tight">Included In Every Bib</h2>
          <div className="mt-3 h-1 w-16 mx-auto rounded-full bg-navy dark:bg-primary-light" />
        </div>
        <div className="grid grid-cols-3 md:grid-cols-3 lg:grid-cols-6 gap-3 md:gap-6">
          {items.map(({ icon: Icon, title, desc, num }) => (
            ...
            >
              ...
              <div className="mx-auto grid place-items-center h-10 w-10 md:h-14 md:w-14 rounded-xl bg-navy/5 text-navy group-hover:bg-gradient-to-br group-hover:from-navy group-hover:to-charcoal group-hover:text-white transition-all duration-500 shadow-sm group-hover:shadow-glow">
                <Icon className="h-5 w-5 md:h-6 md:w-6 transition-transform duration-300" strokeWidth={1.75} />
              </div>
